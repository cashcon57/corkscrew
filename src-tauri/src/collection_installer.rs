...
async fn stage_and_deploy(
    ...
    // fileOverrides — files to SKIP from this mod's deployment so that the
        ...
        let filtered: Vec<String> = files_to_deploy
            ...
            .filter(|f| {
                ...
                let rel = f
                    ...
                    .trim_start_matches('\\')
                    .replace('\\', "/");
                let should_skip = mod_entry.file_overrides.iter().any(|ov| {
                    let norm_ov = ov.replace('\\', "/");
                    rel == norm_ov || rel.ends_with(&format!("/{}", norm_ov))
                });
                if should_skip {
                    log::info!(
                        "fileOverride: skipping '{}' from mod '{}' (other mod wins)",
                        rel,
                        mod_name
                    );
                }
            ...
            .collect();
        log::info!(
            "fileOverrides: excluded {} of {} files from mod '{}'",
            before - filtered.len(),
            before,
            mod_name
        );
