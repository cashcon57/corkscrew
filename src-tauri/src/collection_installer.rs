...
pub struct ModInstallDetail {
    pub name: String,
    pub status: String, // "installed", "skipped", "failed", "user_action"
...
pub async fn install_collection(
    ...
    for (i, &mod_idx) in install_order.iter().enumerate() {
        ...
        let result = install_single_mod(
            app, db, mod_entry, i, game_id, bottle_name, game_slug,
            &data_dir, &download_dir, &api_key, is_premium,
        ).await;
        ...
        match result {
            ...
            Err(InstallError::UserAction { action, url, instructions }) => {
                let _ = app.emit(INSTALL_PROGRESS_EVENT, InstallProgress::UserActionRequired {
                    mod_index: i,
                    mod_name: mod_name.clone(),
                    action: action.clone(),
                    url: url.clone(),
    ...
    if game_id == "skyrimse" {
        ...
        if let Some(pf) = plugins_file {
            ...
            let _ = plugins::skyrim_plugins::sync_plugins(
                Path::new(&game.data_dir), &pf, &loadorder_file,
            );
        }
    }

    // Emit collection completed summary
    let _ = app.emit(INSTALL_PROGRESS_EVENT, InstallProgress::CollectionCompleted {
        installed,
        skipped,
        failed,
    });
...
async fn install_single_mod(
    ...
    match source_type {
        "nexus" => {
            install_nexus_mod(
                app, db, mod_entry, mod_index, game_id, bottle_name,
                game_slug, data_dir, download_dir, api_key, is_premium,
            ).await
        }
        "direct" => {
            install_direct_mod(
                app, db, mod_entry, mod_index, game_id, bottle_name,
                data_dir, download_dir, api_key,
            ).await
        }
        "browse" | "manual" => {
            Err(InstallError::UserAction {
                ...
                instructions: mod_entry.instructions.clone()
...
async fn install_nexus_mod(
    ...
    let nexus_mod_id = mod_entry.source.mod_id.ok_or_else(|| {
        InstallError::Failed(format!("No Nexus mod ID for '{}'", mod_entry.name))
    })?;
    ...
    let key = api_key.as_ref().ok_or_else(|| {
        InstallError::Failed("No API key configured".to_string())
    })?;
    ...
    let _ = app.emit(INSTALL_PROGRESS_EVENT, InstallProgress::StepChanged {
        ...
        detail: Some(format!("Getting download links for '{}'...", mod_entry.name)),
    });
    ...
    let archive_path = client
        ...
        .await
        .map_err(|e| InstallError::Failed(format!("Download failed: {}", e)))?;

    // Stage and deploy the downloaded archive
    let mod_id = stage_and_deploy(
        app, db, &archive_path, mod_entry, mod_index,
        game_id, bottle_name, data_dir,
    )?;
    ...
    let _ = db.set_source_url(
        mod_id,
        &format!("https://www.nexusmods.com/{}/mods/{}", game_slug, nexus_mod_id),
    );
...
async fn install_direct_mod(
    ...
    let url = mod_entry.source.url.as_ref().ok_or_else(|| {
...
fn stage_and_deploy(
    ...
    // Reserve DB record
    let next_priority = db.get_next_priority(game_id, bottle_name)
        .map_err(|e| InstallError::Failed(e.to_string()))?;
    let mod_id = db
        .add_mod(
            game_id, bottle_name, None, mod_name, &mod_entry.version,
    ...
    let files_to_deploy = if let Ok(Some(fomod_installer)) = fomod::parse_fomod(&staging_result.staging_path) {
        ...
        let selections = if let Some(ref choices) = mod_entry.choices {
            ...
            parse_fomod_choices(choices).unwrap_or_else(|| fomod::get_default_selections(&fomod_installer))
        } else {
        ...
        let selected_files = apply_fomod_to_staging(
            &staging_result.staging_path, &fomod_files,
    ...
    if let Err(e) = deployer::deploy_mod(
        ...
        &staging_result.staging_path, data_dir, &files_to_deploy,
    ) {
...
fn parse_fomod_choices(choices: &serde_json::Value) -> Option<std::collections::HashMap<String, Vec<String>>> {
