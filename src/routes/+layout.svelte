<script lang="ts">
  ...
  async function handleDismissInterrupted() {
    ...
    if (interruptedWj) {
      const wj = interruptedWj;
      interruptedWj = null;
      try { await dismissWabbajackInstall(wj.install_id); } catch (e) { console.error('dismissWabbajackInstall:', e); }
    }
  }
  ...
  <nav class="sidebar" class:collapsed={$sidebarCollapsed} class:resizing style={!$sidebarCollapsed ? `width:${sidebarWidth}px;min-width:${sidebarWidth}px` : ''}>
    ...
    <div class="sidebar-footer">
      ...
      <div class="queue-section">
        ...
        <button
          class="queue-btn"
          onclick={() => openUrl("https://discord.gg/nufkm2b7pB")}
          title="Join the Corkscrew Discord"
          aria-label="Join the Corkscrew Discord"
        >
